...
public class SecurityFilter implements Filter{
	...
	if (session == null) {
	    ...
	    return true;
	} else if (session.getAttribute("SPRING_SECURITY_CONTEXT") == null && session.getAttribute("email") != null) {
	    LOGGER.warn("Password has been expired OR need to be changed (first time loggedin)");
	    ((HttpServletResponse) response).sendError(HttpServletResponse.SC_UNAUTHORIZED, "redirect to password change");
	    return true;
	}
